...
test: test-ebs-csi
	cd agent && GO111MODULE=on GOEXPERIMENT=nocoverageredesign ${GOTEST} ${VERBOSE} -tags unit -mod vendor -coverprofile ./cover.out -timeout=120s ./... && go tool cover -func ./cover.out > ../coverprofile.out && cd ..
	cd ecs-agent && GO111MODULE=on GOEXPERIMENT=nocoverageredesign ${GOTEST} ${VERBOSE} -tags unit -mod vendor -coverprofile ./cover.out -timeout=120s ./... && go tool cover -func ./cover.out > ../coverprofile-ecs-agent.out && cd ..
...
test-silent: test-ebs-csi
	cd agent && GO111MODULE=on GOEXPERIMENT=nocoverageredesign ${GOTEST} -tags unit -mod vendor -coverprofile ./cover.out -timeout=120s ./... && go tool cover -func ./cover.out > ../coverprofile.out && cd ..
	cd ecs-agent && GO111MODULE=on GOEXPERIMENT=nocoverageredesign ${GOTEST} -tags unit -mod vendor -coverprofile ./cover.out -timeout=120s ./... && go tool cover -func ./cover.out > ../coverprofile-ecs-agent.out && cd ..
